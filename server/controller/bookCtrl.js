    ...
    createBookingRequest: async (req, res) => {
        try {
            ...
            if (!room.owner || !room.owner.email) {
                return res.status(500).json({ msg: "Room owner information is incomplete" });
            }
    ...
    createResponse: async (req, res) => {
        try {
            ...
            await sendEmail(
                ...
                `Your booking request for the room on ${booking.bookingDate} at ${booking.bookingTime} has been ${status}. Admin's response: ${responseMessage || 'No message'}.`
            );

            res.status(200).json({ msg: 'Booking response updated and user notified' });
        } catch (error) {
